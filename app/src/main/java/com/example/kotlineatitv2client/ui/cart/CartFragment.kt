...
//        cloudFunctions = RetrofitCloudClient.getInstance(Common.currentRestaurant!!.uid).create(ICloudFunctions::class.java)
        ...
        btn_place_order.setOnClickListener {
            ...
            places_fragment = requireActivity().supportFragmentManager.findFragmentById(R.id.places_autocomplete_fragment)
...
//                        }
        ...
        FirebaseDatabase.getInstance()
            ...
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
                        val location = snapshot.getValue(RestaurantLocationModel::class.java)
    ...
    private fun applyShippingCostByLocation(location: RestaurantLocationModel?) {
                        ...
                        override fun onSuccess(totalPrice: Double) {
                            ...
                            if (currentLocation != null)
                            {
                                ...
                                if (location != null) {
                                    ...
                                    orderLocation.latitude = currentLocation!!.latitude
                                    orderLocation.longitude = currentLocation!!.longitude

                                    val restaurantLocation = Location("")
                                    restaurantLocation.latitude = location.lat
                                    restaurantLocation.longitude = location.lng

                                    val distance = orderLocation.distanceTo(restaurantLocation)/1000    // in km
                                    // TODO

                                    if (distance * Common.SHIPPING_COST_PER_KM > Common.MAX_SHIPPING_COST) {
                                        order.shippingCost = Common.MAX_SHIPPING_COST.toDouble()    // if to far, we will get max value
                                    } else {
                                        order.shippingCost = (distance * Common.SHIPPING_COST_PER_KM).toDouble()
                                    }
                                }else {
                                    order.shippingCost = 0.toDouble()
    ...
    override fun onSearchFound(category: CategoryModel, cartItem: CartItem) {
        ...
        if (foodModel != null)
            ...
            Toast.makeText(requireContext(),"Food Id not found",Toast.LENGTH_SHORT).show()
    }

    private fun showUpdateDialog(cartItem: CartItem, foodModel: FoodModel) {
        Common.foodSelected = foodModel
        val builder = AlertDialog.Builder(requireContext())
        val itemView:View = LayoutInflater.from(requireContext()).inflate(R.layout.layout_dialog_update_cart,null)
